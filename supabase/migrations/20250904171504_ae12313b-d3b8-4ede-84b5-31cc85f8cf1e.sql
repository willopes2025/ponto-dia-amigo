...
BEGIN
  ...
  SELECT sh.hora_inicio, sh.hora_fim
  ...
  IF v_hora_inicio IS NOT NULL AND v_hora_fim IS NOT NULL THEN
    v_shift_hours := v_hora_fim - v_hora_inicio;
  ELSE
    v_shift_hours := INTERVAL '8 hours'; -- Default 8 hours
  END IF;
  ...
  VALUES (
    ...
    v_shift_hours,
    CASE 
      WHEN v_entrada IS NOT NULL AND v_saida IS NOT NULL THEN 'completo'::daily_status
      ELSE 'incompleto'::daily_status
    END,
